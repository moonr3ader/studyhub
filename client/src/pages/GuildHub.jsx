...
const GuildHub = () => {
  ...
  const handleForgeGuild = async (e) => {
    ...
    try {
      ...
      // Success! Close modal, clear form, and refresh the guild list
      setShowModal(false);
      setNewGuildName('');
      setNewGuildDesc('');
      fetchGuilds(); 
  ...
  const handleJoinGuild = async (guildId) => {
    ...
    try {
      ...
      fetchGuilds(); // Refresh the grid to instantly update the member count (e.g., 1/5 becomes 2/5)
      
    } catch (err) {
      // This will display our custom backend errors (e.g., "Already in a Guild!" or "Guild Full")
    ...
    <div className="min-h-screen bg-[#0B0E14] text-slate-200 p-10 relative">
      <div className="max-w-6xl mx-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {guilds.length === 0 ? (
            <p className="text-slate-500 col-span-full text-center py-10">No guilds have been forged yet. Be the first!</p>
          ) : (
            guilds.map((guild) => (
              <div key={guild._id} className="bg-[#161B22] border border-white/5 rounded-3xl p-6 hover:border-purple-500/30 transition-all group flex flex-col">
                <div className="flex justify-between items-start mb-4">
                  ...
                  <span className="bg-[#0B0E14] text-slate-400 px-3 py-1 rounded-lg text-xs font-mono border border-white/5 flex items-center gap-2">
                    <Users size={12} /> {guild.members?.length || 1}/5
                  </span>
                </div>
                <p className="text-slate-400 text-sm mb-6 flex-1">{guild.guildDescription}</p>
                <button 
                  onClick={() => handleJoinGuild(guild._id)}
                  className="w-full bg-[#0B0E14] hover:bg-emerald-600/20 text-emerald-500 border border-emerald-500/30 hover:border-emerald-500 py-3 rounded-xl font-bold transition-all flex items-center justify-center gap-2"
                >
            ...
            <form onSubmit={handleForgeGuild} className="space-y-6">
              <div>
                ...
                <input 
                  ...
                  className="w-full bg-[#0B0E14] border border-slate-700 rounded-xl p-3 outline-none focus:border-purple-500 text-white"
                  placeholder="e.g., Syntax Sorcerers"
                />
              </div>
              <div>
                <label className="text-xs font-bold uppercase tracking-widest text-slate-500 block mb-2">Guild Creed (Description)</label>
                <textarea 
                  required rows="3"
                  value={newGuildDesc} onChange={(e) => setNewGuildDesc(e.target.value)}
                  className="w-full bg-[#0B0E14] border border-slate-700 rounded-xl p-3 outline-none focus:border-purple-500 text-white resize-none"
                  placeholder="What is your team's focus?"
                ></textarea>
              </div>
              <button type="submit" className="w-full bg-purple-600 hover:bg-purple-500 py-4 rounded-xl font-black uppercase tracking-widest text-white transition-transform active:scale-95">
                Establish Guild
              </button>
            </form>
          </div>
